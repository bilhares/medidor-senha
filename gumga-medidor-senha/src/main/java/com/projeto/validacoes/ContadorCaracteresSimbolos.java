...
public class ContadorCaracteresSimbolos implements RegraCalculoCaracteres {
	...
	public Caracter calcular(Caracter c, String senha) {
		...
		for (int i = 0; i < senha.length(); i++) {
			...
			if (caracter.toString().matches(PatternRegex.SIMBOLOS.getPattern())) {
		...
		CaracterSimbolos retorno = new CaracterSimbolos();
